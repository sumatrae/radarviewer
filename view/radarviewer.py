...
class Radar_Viewer(QMainWindow):
    def __init__(self, *args):
        ...
        self.radar_viewer = loadUi("radar_viewer.ui", self)
        ...
        self.cam_manager.set_camera_id(1)
...
class CV_Thread(QThread):
    ...
    def run(self):
        while 1:
            ...
            scaled_pixmap = pixmap.scaled(frame.shape[1],frame.shape[0],aspectRatioMode=Qt.KeepAspectRatioByExpanding)

            self.radar_viewer.label_video.setPixmap(scaled_pixmap)
            self.radar_viewer.label_video.setScaledContents(True)
